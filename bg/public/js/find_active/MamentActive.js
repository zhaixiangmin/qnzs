...
function usdershow(n) {
	...
	$("#imghead").attr('src', '');
	$(".signTimes input[name='rd']:checked").attr('checked', '');
	$('input[name="isAddTemplate"]:checked').attr('checked', '');
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.status != 'OK') {
					...
					// update_flay =true ;  //解锁
					$(".signTimes input[name='rd']:checked").attr('checked', ''); //获取签退次数                                           //签退
...
function terminate(n) {
	...
	if(row) {
		...
		$('#btn_imglook').click(function() {
			...
			$.ajax({
				type: "POST",
				url: Qnzs.path + "/activity/offlineActivity/bg/terminate",
				data: data,
				dataType: "JSON",
				success: function(data) {
					if(data.status != 'OK') {
