	...
	},*/
	{
		...
		formatter: function(value, row, index) {
			if (row.fileId) {
				...
				return '<a href="' + Qnzs.path + '/downLoadFile?fileId=' + row.fileId + '&fileName=' + row.fileName + '">下载</a>';
			} else {
				return '';
			}
		}
...
function exportData(n) {
	var pageIndex = $('#statistics_table').datagrid("getPager").pagination("options").pageNumber;
	var pageSize = $('#statistics_table').datagrid("getPager").pagination("options").pageSize;

	window.location.href = Qnzs.path + '/activity/enroll/bg/enrolledAcc/export?activityId=' + activityId + '&pageIndex=' + pageIndex + '&pageSize=' + pageSize;
}