  ...
  if(type == 2) { // 2-扫码评分(未预约)，获取到站点ID
    ...
    $.ajax({
      ...
      success:function(data){
        ...
        // 测试数据
        station = {
          imageUrl: 'https://wgj-web-admin.oss-cn-shenzhen.aliyuncs.com/temp/image/20180921/20180921103437_325579147553449385282.jpg',
          staCategoriesList: [{
            name: 'app'
          }],
          staSupplyList: {
        ...
        $('#imageUrl').attr('src', Utils.compressByAli(imgUrl, 200, 200)); // 站点头像
        ...
        var supply_html = '';
        for(var i=0; i<station.staSupplyList.length; i++) {
          var staSupply = station.staSupplyList[i];
          supply_html += '<li class="fl category" data-id="' + staSupply.id + '">';
          supply_html += '    <a href="javascript:;" class="colorfff fz24">';
          supply_html += '        <div>' + staSupply.name + '</div>';
          supply_html += '    </a>';
          supply_html += '</li>';
        }
        $('#staCategoriesList').html(supply_html); // 渲染服务快捷选项
    ...
    $('#submit_appreciation_unreserve').click(function () {
      ...
      //     return;
      // }
      if(params.evaluate && params.evaluate.length < 5) {
        $.alert('够5个字才能提交哦！');
        return;
      }
      if(!params.score) {
