...
$(function () {
    ...
    function render_servicelist($listContent, services, isClear) {
        ...
        for (var i = 0; i < services.length; i++) {
            ...
            // html += '<a href="../person_center/release_offline_service_detail.html?staId=' + service.stationId + '&apId=' + service.apId + '" class="item clearfix disB">';
            html += '<a href="../person_center/release_offline_service_detail.html?applicationId=' + service.apId + '" class="item clearfix disB">';
            html += '    <div class="left fl">';
            html += '        <img src="' + Utils.compressByAli(imgUrl, 160, 200) + '"/>';
            html += '    </div>';
            html += '    <div class="right">';
            html += '        <h3 class="color000 fz30">' + service.quesTitle + '</h3>';
            html += '        <p class="fz26 color666">' + service.statusStr + '</p>';
            html += '        <div class="botTxt clearfix">';
            html += '            <span class="color999 fz24 fl">' + service.applyTime + '</span>';
            html += '        </div>';
            html += '    </div>';
            html += '</a>';
        }
    ...
    function loadServiceList(fun, params, $listContent, isClear) {
        ...
        fun(params).then(function (data) {
            ...
            if(params.pageNo == 1) { // 第一页
                $('#release_offservice_service_total').append('(' + data.total + ')'); // 帮助名称后面添加总记录数
            }

            render_servicelist($listContent, services, isClear); // 渲染帮助列表
            if(services && services.length >= params.pageSize) { // 全部列表数据尚未查询完毕
    ...
    loadServiceList(PersonCenterApi.getMyApplications, params, $('.case .content')); // 加载帮助列表并渲染页面
    ...
    //         return;
    //     }
    //     loadServiceList(PersonCenterApi.getMyApplications, params, $('.case .content'), true); // 加载帮助列表并渲染页面
    // });
