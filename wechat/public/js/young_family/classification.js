 ...
 */
$(function () {
    var caId = Utils.getQueryString('caId'); // 服务类别ID
    var name = Utils.getQueryString('name'); // 服务类别名称
    console.log('caId', caId);
    if(!caId) {
        $.alert('服务类别ID不能为空').then(function () {
            // window.location.href = 'young_family.html';
            window.history.back(); // 返回上一页
        });
    }

    $('#stationName').text(name); // 渲染站点名称
    ...
    function loadStationList(fun, params, $listContent) {
        ...
        fun(params).then(function (data) {
            ...
            console.log('YoungFamilyApi.getWapRecommendStations data', data);
    ...
    loadStationList(YoungFamilyApi.getWapRecommendStations, params, $('#popularity')); // 加载站点列表并渲染页面
