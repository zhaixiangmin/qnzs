...
$(function () {
    ...
    $('#siteBox').on('click', '.list_item', function () {
    	//清空
       $.cookie('district_myinfo', "", {path: '/'}); // 存储到cookie(区域)
        // 存储到cookie
        var district_myinfo = {
        ...
        if(!district_myinfo.sitenavOrgId || !district_myinfo.sitenavOrgName) {
            $.alert('区域为空');
            return;
        }

        district_myinfo = JSON.stringify(district_myinfo);
        $.cookie('district_myinfo', district_myinfo, {path: '/'}); // 存储到cookie(区域)

        // window.history.back(); // 返回上一页面
        var refreshMs = (new Date()).getTime();
        //window.location.href = '../../index.html?refresh=' + refreshMs; // 防止苹果手机不自动刷新
        window.location.href="../wapUserInfo.html";
    });
