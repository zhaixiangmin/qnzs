...
$(function () {
    ...
    Qnzs.getSessionAccount({}).then(function (data) {
        ...
        if(account_common.orgType == 3) { // 0-超级管理员，1-系统管理员，2-组织管理员，3-个人用户
            $('#registerBtn .disB').text(account_common.realname); // 注册/(用户名/管理后台)
            return;
        }

        // 非个人用户
        $('#registerBtn .disB').text('管理后台'); // 注册/(用户名/管理后台)
        $('#bell_login').show(); // 显示铃铛图标
    });
    ...
    function render(tabs, tabindex_qnzs) {
        ...
        header += '<span class="disB">我要吐槽</span>';
        header += '</a>';
        header += '<a href="javascript:;" class="fl" id="bell_login" style="padding-top: 16px;padding-bottom: 10px;display: none;">';
    ...
    var tabs = [
        ...
        { name: '重磅项目', path: '../heavy_project/heavy_project_model1_index.html'},
