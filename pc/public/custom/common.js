...
function getAccessTokenAndLogin(random){
    ...
    Qnzs.getSessionAccount({}).then(function (data) {
        ...
        if(account_common.orgType == 0 || account_common.orgType == 1 || account_common.orgType == 2) { // 0-超级管理员，1-系统管理员，2-组织管理员，3-个人用户
            ...
            //$('#registerBtn').attr('href' ,'view/person_center/person_center.html'); // 注册/(用户名)
    ...
    function render(tabs) {
        ...
        header += '<a href="//izhanchi.com/" target="_blank" class="fl a03">易展翅</a>';
        header += '<a href="//www.gdtuanju.com/" style="display:none;"   target="_blank" class="fl a04">粤团聚</a>';
        header += '</div>';
        ...
        footer += '                <span id="_ideConac"><a href="http://bszs.conac.cn/sitename?method=show&id=5418DCF51DDE4A7BE053012819AC5498">';
        footer += '					<img style="width: 60px;" src="' + prefix + 'public/img/index_footer.png"></a></span>';
        footer += '            </div>';
        ...
        footer += '                <a href="http://www.miibeian.gov.cn/" target="_blank" style="color: #fff;">粤ICP备10214402号-4</a>';
        footer += '            </div>';
        footer += '        </div>';
        footer += '    </div>';
        footer += '</footer>';
    ...
    function login_box() {
        ...
        html += '                    <input type="text" class="input_txt" id="user_name" placeholder="请输入手机号" />';
        html += '                </div>';
        html += '                <div class="input_item pwd">';
        html += '                    <input type="password" id="user_pwd" class="input_txt"  placeholder="请输入登录密码" />';
        html += '                </div>';
        html += '                <div class="input_check_casd_box clearfix" style="display: none;">';
...
//
        ...
        $.ajax({
            type:"post",
            url: Qnzs.path+"/pc/account/login",
            async:true,
            data:data,
            xhrFields: {
                ...
                if(data.status=="ERROR"){
                    ...
                    if(data.errorcount){
                        if(data.errorcount>2){
                            $('.input_check_casd_box').show() //显示图形验证码
                        }else{
                            $('.input_check_casd_box').hide() //隐藏图形验证码
                        }
                    }
                }else {    //成功登录
                    //获取cookie的安全连接
