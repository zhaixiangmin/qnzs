...
$(function () {
    ...
    function render(tabs) {
        ...
        var sitenavOrgIdSuffix = '?sitenavOrgId=440000'; // 区域ID后缀(默认广东)
        var districtName = '广东'; // 区域名称
        if($.cookie && $.cookie('district_qnzs')) {
            var district_qnzs = $.cookie('district_qnzs'); // 获取cookie(区域)
            district_qnzs = JSON.parse(district_qnzs);
            // //console.log('获取 district_qnzs', district_qnzs);
            sitenavOrgIdSuffix = '?sitenavOrgId=' + district_qnzs.sitenavOrgId;
            districtName = district_qnzs.sitenavOrgName; // 区域名称
        }
        ...
        header += '</div>';
        header += '<a href="javascript:;" class="disIB colorfff font14 didian">' + districtName + '</a>';
        header += '</div>';
        ...
        for(var i=0; i<tabs.length; i++) { // 选项卡加载
            ...
            if(isNginx) { // nginx环境
                if(isIndexPage) { // 当前页是首页
                    ...
                    if(i == 0) { // 首页选项卡
                        header += '<li><a href="/pc' + tab.path + sitenavOrgIdSuffix + '" class="disB" style="cursor: pointer;">' + tab.name + '</a></li>';
                    }else { // 非首页选项卡(/view/find_help/find_help.html)
                        var keyword = tab.path.split('/')[1]; // ['', 'find_help', 'find_help.html'] --> find_help
                        //console.log('keyword', keyword);
                        if(pathname.indexOf(keyword) != -1) { // 高亮当前页面选项卡
                            header += '<li><a href="/pc/view' + tab.path + sitenavOrgIdSuffix + '" class="disB conBgc02" style="cursor: pointer;">' + tab.name + '</a></li>';
                            continue;
                        }

                        // 去高亮当前页面选项卡
                        header += '<li><a href="/pc/view' + tab.path + sitenavOrgIdSuffix + '" class="disB" style="cursor: pointer;">' + tab.name + '</a></li>';
                    }
                }
            }else { // 本地开发
                if(isIndexPage) { // 当前页是首页
                    if(i == 0) { // 首页选项卡
                        ...
                        header += '<li><a href="view' + tab.path + sitenavOrgIdSuffix + '" class="disB" style="cursor: pointer;">' + tab.name + '</a></li>';
                    }
                }else { // 当前页非首页
                    if(i == 0) { // 首页选项卡 ../../index.html
                        header += '<li><a href="../..' + tab.path + sitenavOrgIdSuffix + '" class="disB" style="cursor: pointer;">' + tab.name + '</a></li>';
                    }else { // 非首页选项卡(/view/find_help/find_help.html)
                        var keyword = tab.path.split('/')[1]; // ['', 'find_help', 'find_help.html'] --> find_help
                        // //console.log('keyword', keyword);
                        if(pathname.indexOf(keyword) != -1) { // 高亮当前页面选项卡
                            header += '<li><a href="..' + tab.path + sitenavOrgIdSuffix + '" class="disB conBgc02" style="cursor: pointer;">' + tab.name + '</a></li>';
                            continue;
                        }
    ...
    function render_school(params_school) {
        Qnzs.getDistrictByType(params_school).then(function (data) {
            ...
            for(var i=0; i<schools.length; i++) {
                ...
                html += '<a class="district" data-id="' + school.did + '" data-name="' + school.districtName + '">' + school.districtName + '</a>';
            }
    ...
    $('body').on('click', '.popup .citylist .district', function () {
       ...
       var sitenavOrgName = $(this).data('name');
        ...
        if($.cookie) {
            // $.cookie('sitenavOrgId', sitenavOrgId, {path: '/'}); // 存储到cookie(区域导航ID)
            // $.cookie('sitenavOrgName', sitenavOrgName, {path: '/'}); // 存储到cookie(区域导航名称)
            var district_qnzs = {
                sitenavOrgId: sitenavOrgId,
                sitenavOrgName: sitenavOrgName
            };
            district_qnzs = JSON.stringify(district_qnzs);
            //console.log('district_qnzs', district_qnzs);
