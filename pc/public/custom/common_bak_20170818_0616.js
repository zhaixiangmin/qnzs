...
$(function () {
    ...
    if(isNginx) { // nginx环境
        if($.cookie && $.cookie('district_qnzs')) { // 有cookie
            ...
            // render(tabs, district); // 渲染公共头部、尾部
            ...
            if(hostArr.length >= 3) {
                ...
                if(regionName == 'www'){ // 正常域名
                    // district.sitenavOrgId = 440000; // 区域ID(广东)
                    // district.sitenavOrgName = '广东'; // 区域名称(广东)
                    // 获取用户区域ID
                    Qnzs.getDistrictIdByUserDistrictId({}).then(function (data) {
                        if(data.data) {
                            ...
                            // district.sitenavOrgId = 440000; // 区域ID
                            // district.sitenavOrgName = '广东'; // 区域名称
                        }
                    }).always(function () {
                        // 存储到cookie
                        var district_qnzs = {
